...
if len(sys.argv)!=5:
	print '\n'+'\n'+'\n'+'To use this script, type: python batch_blastn.py {directory of query sequences[USE THE FULL PATH WITH NO END SLASH]} {output directory [USE THE FULL PATH AND NO END SLASH} {results file name} {BLAST database name}\n'
	if len(sys.argv)<5:
...
results_file = open (sys.argv[3],'wt')
print results_file
...
for filename in files:
	if filename.endswith('.fa'):
		...
		blastn_cmd='blastn -db '+sys.argv[4]+' -query '+sys.argv[1]+'/'+filename+' -outfmt "6 sseqid evalue bitscore pident qlen slen qstart qend sstart send sseq">'+sys.argv[2]+'/'+filename.rstrip('.fa')+'_blastn_results.txt'
